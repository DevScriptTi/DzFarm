...
export async function isFarmer(): Promise<boolean> {
  try {
    const user = await getUser();
    return user?.user?.key.keyable_type === "farmer";
  } catch (error) {
...
export async function isVeterinary(): Promise<boolean> {
  try {
    const user = await getUser();
    return user?.user?.key.keyable_type === "veterinary";
  } catch (error) {
    console.error("Veterinary check failed:", error);
    return false;
  }
}
